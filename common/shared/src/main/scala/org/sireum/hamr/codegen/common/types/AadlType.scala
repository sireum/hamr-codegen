...
@datatype class DataTypeNames(typ: AadlType,
  ...
  def payloadName: String = { return if(typ == TypeUtil.EmptyType) typeName else s"${typeName}_Payload" }
  ...
  def isEmptyType(): B = { return typ == TypeUtil.EmptyType }

  def isBitsTypes(): B = { return typ == TypeUtil.SlangEmbeddedBitType }
