        ...
        case Some(GclCompute(modifies, assumes, guarantees, cases, handlers, flows, gumboTables)) => {
          ...
          for (table <- s.compute.get.gumboTables) {// for each table.
            ...
            var hpreds: ISZ[Exp] = for(p <- table.table.horizontalPredicates) yield typeCheckBoolExp(
              exp=p,context=context,mode=TypeChecker.ModeContext.SpecPost,
              component=Some(component),params=ISZ(),stateVars=s.state,
              specFuns=gclMethods,symbolTable=symbolTable,aadlTypes=aadlTypes,scope=scope,typeHierarchy=typeHierarchy,reporter=reporter) // Type checked.
            var vpreds: ISZ[Exp] = for(p <- table.table.verticalPredicates) yield typeCheckBoolExp(
              exp=p,context=context,mode=TypeChecker.ModeContext.SpecPost,
              component=Some(component),params=ISZ(),stateVars=s.state,
              specFuns=gclMethods,symbolTable=symbolTable,aadlTypes=aadlTypes,scope=scope,typeHierarchy=typeHierarchy,reporter=reporter)
            var rres: ISZ[ISZ[Exp]] =
              for(row: GclResultRow <- table.table.resultRows) yield (
                for(r:Exp <- row.results) yield typeCheckBoolExp(
                  exp=r,context=context,mode=TypeChecker.ModeContext.SpecPost,
                  component=Some(component),params=ISZ(),stateVars=s.state,
                  specFuns=gclMethods,symbolTable=symbolTable,aadlTypes=aadlTypes,scope=scope,typeHierarchy=typeHierarchy,reporter=reporter)
                )
