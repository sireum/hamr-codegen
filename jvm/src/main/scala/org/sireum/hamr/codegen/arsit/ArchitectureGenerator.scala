...
object ArchitectureGenerator {
  ...
  val KEY_TOUCHED_TYPES_SLANG: String = "KEY_TOUCHED_TYPES_SLANG"

  @strictpure def getTouchedTypesOpt(store: Store): Option[TouchedTypes] = store.get(KEY_TOUCHED_TYPES_SLANG).asInstanceOf[Option[TouchedTypes]]

  @strictpure def getTouchedTypes(store: Store): TouchedTypes = getTouchedTypesOpt(store).get

  @datatype class TouchedTypes(orderedDependencies: ISZ[String]) extends StoreValue
}

@record class ArchitectureGenerator {
  var componentId: Z = 0
  var portId: Z = 0
  ...
  def generate(rootSystem: AadlSystem, directories: ProjectDirectories, plugins: ISZ[Plugin], arsitOptions: ArsitOptions, aadlTypes: AadlTypes, symbolTable: SymbolTable, store: Store): (Result, Store) = {
    var localStore = store
    val basePackage = arsitOptions.packageName
    if (!aadlTypes.rawConnections) {
      // TODO allow for customizations of base types
      val orderedTypeDependencies = getTouchedTypes(arsitOptions, aadlTypes, symbolTable, reporter)
      localStore = localStore + ArchitectureGenerator.KEY_TOUCHED_TYPES_SLANG ~> ArchitectureGenerator.TouchedTypes(orderedTypeDependencies)

      for (t <- orderedTypeDependencies) {
        val aadlType = aadlTypes.typeMap.get(t).get
        ...
        if (!aadlType.isInstanceOf[BaseType]) {
          ...
          for (p <- plugins if p.isInstanceOf[DatatypeProviderPlugin] &&
            ...
            p.asInstanceOf[DatatypeProviderPlugin].handleDatatypeProvider(
              ...
              directories, aadlTypes, symbolTable, localStore, reporter) match {
    ...
    generateInternal(basePackage, rootSystem, directories, arsitOptions, localStore, aadlTypes, symbolTable)
  ...
  def generateInternal(basePackage: String, rootSystem: AadlSystem, directories: ProjectDirectories, arsitOptions: ArsitOptions, store: Store, aadlTypes: AadlTypes, symbolTable: SymbolTable): Unit = {
    gen(rootSystem, basePackage, arsitOptions, aadlTypes, symbolTable)
    ...
    val touchMethod: Option[ST] =
      if (NixGen.willTranspile(arsitOptions.platform)) {
        ...
        val apiTouches = NixGen.genApiTouches(aadlTypes, basePackage, components)
  ...
  def gen(c: AadlComponent, basePackage: String, arsitOptions: ArsitOptions, aadlTypes: AadlTypes, symbolTable: SymbolTable): Unit = {
    c match {
      case s: AadlSystem => genContainer(s, basePackage, arsitOptions, aadlTypes, symbolTable)
      case s: AadlProcess => genContainer(s, basePackage, arsitOptions, aadlTypes, symbolTable)

      case s: AadlThreadGroup => genThreadGroup(s, basePackage, arsitOptions, aadlTypes, symbolTable)

      case _ =>
        for (_c <- c.subComponents) {
          gen(_c, basePackage, arsitOptions, aadlTypes, symbolTable)
        }
    }
  }

  def genContainer(m: AadlComponent, basePackage: String, arsitOptions: ArsitOptions, aadlTypes: AadlTypes, symbolTable: SymbolTable): Unit = {
    ...
    for (c <- m.subComponents) {
      c match {
        ...
        case s: AadlThread =>
          ...
          bridges = bridges :+ genThread(s, basePackage, names, arsitOptions, aadlTypes)
    ...
    connections = connections ++ m.connectionInstances
      .filter((ci: ir.ConnectionInstance) => allowConnection(ci, m.component, arsitOptions, symbolTable))
      .map((ci: ir.ConnectionInstance) => processConnectionInstance(ci, symbolTable))
  }


  def genThreadGroup(m: AadlThreadGroup, basePackage: String, arsitOptions: ArsitOptions, aadlTypes: AadlTypes, symbolTable: SymbolTable): Unit = {

    for (c <- m.subComponents) {
      assert(c.isInstanceOf[AadlThread])
      val names = nameProvider(c.component, basePackage)
      bridges = bridges :+ genThread(c.asInstanceOf[AadlThread], basePackage, names, arsitOptions, aadlTypes)
      components = components :+ names.instanceName
    }
  ...
  def genThread(m: AadlThreadOrDevice, basePackage: String, names: NameProvider, arsitOptions: ArsitOptions, aadlTypes: AadlTypes): ST = {
    ...
    val ports: ISZ[Port] = Util.getPorts(m, aadlTypes, basePackage, portId)
  ...
  def processConnectionInstance(ci: ConnectionInstance, symbolTable: SymbolTable): ST = {
  ...
  def allowConnection(c: ConnectionInstance, srcComponent: ir.Component, arsitOptions: ArsitOptions, symbolTable: SymbolTable): B = {
  ...
  @pure def getTouchedTypes(arsitOptions: ArsitOptions, aadlTypes: AadlTypes, symbolTable: SymbolTable, reporter: Reporter): ISZ[String] = {
    ...
    return TypeUtil.orderTypeDependencies(ret.elements)
  }
}
