...
object GumboGen {
  ...
  @datatype class GclEntryPointPeriodicCompute(val markers: ISZ[Marker],
                                               ...
                                               val ensures: Option[ST],
                                               val flows: Option[ST],
                                               val tables: Option[ST]) extends GclEntryPointContainer
  ...
  def processInitializes(m: AadlThreadOrDevice, symbolTable: SymbolTable, aadlTypes: AadlTypes, basePackage: String): Option[GclEntryPointInitialize] = {
    ...
    if (ais.nonEmpty) {
      ...
      if (sc.initializes.nonEmpty) {
        ...
        return Some(GclEntryPointInitialize(imports, markers, ret, modifies, requires, ensures, flows, tables))
      } else {
        return None()
      }
    } else {
...
@record class GumboGen(gclSymbolTable: GclSymbolTable,
  ...
  def processCompute(compute: GclCompute, optInEvent: Option[AadlPort], context: AadlThreadOrDevice): (ContractBlock, ISZ[Marker]) = {
    ...
    // TODO Meeting List:
    //  - Figure out what to do about deterministic check.
    //  - Initialization handles the contract block differently, adapt to this. (ANS: Axe Init)
    //  - GumboX generation (harder than logika generation)
    //  - Examples/Generation on examples.

    //for each table
    for(t <- compute.tables){
      // TODO match statement on t (Not doing it yet since we only have normal tables)
      //  normal table

      // get normal table
      val nt: NormalTable = t.table
      // horizontal preds
      val hps: ISZ[AST.Exp] = nt.horizontalPredicates
      //vertical preds
      val vps: ISZ[AST.Exp] = nt.verticalPredicates
      // result rows
      val rrs: ISZ[ResultRow] = nt.resultRows
      //
