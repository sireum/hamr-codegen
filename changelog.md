...
**ROS2**
  ...
  * Added platform-provided types and components, decoupling where a type's realization comes from (generated by codegen, or already present on the platform) from how topics are named, so generated nodes can exchange messages with stock ROS 2 nodes rather than forming a closed system: ``Type_Provenance`` plus the structural ``<ros package>::<Message>`` convention with a ``Native_Name`` escape hatch (a platform-provided type gets no ``.msg`` file and no entry in the interfaces package, and its ROS package becomes a build/runtime dependency), and ``Component_Provenance``, where a platform-provided component generates no code at all -- only a launch entry derived from its classifier, a preserved block for launch configuration, and an ``exec_depend`` so rosdep pulls the stock node

  * Model-carried naming: ``Ros_Namespace`` is applied in node init (both backends) and in the launch entry, and ``Ros_Topic_Name`` is resolved per connection rather than per port -- the declaring end keeps its literal so rcl expands it against that node's own namespace, and the peer is bound to the resolved absolute name.  Consistency checks over the graph the topic names induce report type disagreement and data-port fan-in as errors, and a name match with no modeled connection as information, since that is how a generated node joins a stock one

  * ``rosout`` is now a reserved port name denoting every node's built-in log topic: an out ``rosout`` port emits no publisher and no ``put_`` API and is excluded from the rclc entity counts (on micro-ROS its presence is what enables rcl logging), while an in ``rosout`` port takes the ordinary subscription path pinned to ``/rosout`` and its handler deliberately does not log, since a node subscribed to its own log output would feed itself indefinitely

  * micro-ROS nodes now have AADL dispatch semantics: ``put_<port>`` stages a value that ``sendOutputs`` releases at the end of the dispatch, and each queued port yields at most one arrival per dispatch, so ``_hasEvent`` means what this dispatch received rather than what is pending now (the per-port ring buffer is gone).  Because ``--strict-aadl-mode`` does not reach the micro-ROS nodes, codegen now reports the dispatch semantics those nodes actually get, so a mixed system cannot quietly disagree with the command line

  * Strict-mode rclcpp nodes split the single node mutex into ``state_mutex_`` (port queues) and ``dispatch_mutex_`` (dispatch serialization).  The one mutex had to be held across the user's entry point, so any ``put_``/``get_`` from user code deadlocked on a non-recursive mutex; ``state_mutex_`` also covers the ``receiveInputs``/subscription-callback race ThreadSanitizer caught

  * Generated micro-ROS nodes log via ``RCUTILS_LOG_*_NAMED`` with the node's registered logger name instead of ``printf`` (declaring the rcutils dependency), and ``MESSAGE_TO_STRING`` printers are emitted for micro-ROS payloads

  * Checked micro-ROS init: ``<node>_base_init`` returns ``rcl_ret_t`` and checks each rcl/rclc call through an ``RCL_CHECK`` macro, and the runner reports the status and exits non-zero.  rcl/rclc report entity-creation failures by return code rather than by trapping, so continuing past one left a node that spun normally but silently never published or received

  * micro-ROS subscription buffers are statically sized from the mirror's array dimensions and attached before executor init, keeping this heap-free and adding no ``micro_ros_utilities`` dependency; a lint reports payloads codegen could not size, since undersizing is silent at runtime.  Added ``USER DECLARATIONS`` and ``USER INIT`` preserved blocks for fields codegen cannot size from the model, and a ``node_options`` seam via ``rclc_support_init_with_options`` fed by a preserved argv block

  * Python launch files are generated alongside XML, with the same node set, namespaces and deployment split as the XML generator, plus ``LAUNCH PREAMBLE`` and ``LAUNCH OVERRIDE`` preserved blocks that have no XML counterpart; both formats are always emitted

  * The micro-ROS half of a system gets its own launch file, since those nodes are flashed rather than launched and the agent may already be running elsewhere.  Every node entry carries a preserved ``LAUNCH CONFIG`` block for parameters and remappings, and every launch file a file-level ``LAUNCH ARGUMENTS`` block; marker ids rendered into XML comments are made comment-safe, so an edit to the marker prose cannot produce a launch file no XML parser accepts

  * Added ``launch``, ``launch-py`` and ``launch-xml`` Makefile targets, with ``LAUNCH_ARGS`` for ``ros2 launch`` arguments and ``LAUNCH_FILE`` to select an entry launch file among several systems; the generated readme's Quick Start gains ``make build`` and the launch rows

  * ``ros2OutputWorkspaceDir`` now defaults to ``<output-dir>/ros`` and is canonicalized, so the backend always writes under an absolute base instead of emitting paths relative to the working directory

  * micro-ROS firmware build fixes: ``microros-config`` forwards ``--cmake-force-configure`` (with the separator ``ros2 run`` consumes accounted for) so a changed ``colcon.meta`` cannot leave stale CMake caches behind a target that still reports success, ``build-microros`` runs under ``env -i`` so the build neither inherits nor records unrelated colcon prefix chains, ``RMW_UXRCE_MAX_PUBLISHERS`` accounts for the ``/rosout`` publisher ``rcl_node_init`` creates when rcl logging is on, and an existing ``src/colcon.meta`` is backed up only when it is not already codegen's own copy

  * Fixes: the platform-provided ``exec_depend`` entries in the generated bringup ``package.xml`` are no longer mis-indented, and the ``RCL_COMMAND_LINE_ENABLED`` rationale in the emitted ``colcon.meta``, Makefile and readme is corrected

**General**

  * Added a link to the HAMR website

  * Generated CLI sources record ``$SIREUM_HOME``-relative provenance instead of an absolute machine path

**Backward Incompatibilities**

  * **ROS2**

    * C++ is now the default node language -- ``Cpp`` is the first ``NodesCodeLanguage`` choice.  Python node generation is unimplemented and now reports that it is unsupported, so a plain ``--platform ros2`` run no longer fails unless the node language was also given explicitly

    * The generated ``PRINT_INFO`` / ``PRINT_WARN`` / ``PRINT_ERROR`` macros are renamed to ``LOG_INFO`` / ``LOG_WARN`` / ``LOG_ERROR``

    * The ``HAMR_ROS::RosNodeKind`` property is renamed to ``Ros_Node_Kind``, matching the renamed library attribute

    * A ``Queue_Size`` greater than 1 is now rejected by the linter for every generated ROS2 thread, since only the AADL default of 1 is realized by either backend

    * ``ros2LaunchLanguage`` is no longer consulted -- both launch formats are always emitted -- and the option is expected to be dropped
...
```
export SIREUM_V=4.20260810.80aad0c2
```
...
* [3e6ccaf](https://github.com/sireum/hamr-codegen/commit/3e6ccaf) update submodule

* [6bd2d2f](https://github.com/sireum/hamr-codegen/commit/6bd2d2f) Fix the microros-config target and size the publisher pool for /rosout

* [cb5fdc8](https://github.com/sireum/hamr-codegen/commit/cb5fdc8) Move the Queue_Size check into the linter, keep rosout handlers from logging

* [4bc9b27](https://github.com/sireum/hamr-codegen/commit/4bc9b27) Report the dispatch semantics micro-ROS nodes actually get

* [4c72203](https://github.com/sireum/hamr-codegen/commit/4c72203) Split the strict-mode node mutex into state and dispatch locks

* [2c5265b](https://github.com/sireum/hamr-codegen/commit/2c5265b) Emit MESSAGE_TO_STRING printers for micro-ROS payloads

* [82b20a6](https://github.com/sireum/hamr-codegen/commit/82b20a6) Give micro-ROS nodes AADL dispatch semantics, rename PRINT_* to LOG_*

* [a9f0d13](https://github.com/sireum/hamr-codegen/commit/a9f0d13) ignore internal

* [c06e462](https://github.com/sireum/hamr-codegen/commit/c06e462) update submodule

* [9de8a75](https://github.com/sireum/hamr-codegen/commit/9de8a75) handle queue size for event ports

* [450cd4a](https://github.com/sireum/hamr-codegen/commit/450cd4a) update submodule

* [cae980d](https://github.com/sireum/hamr-codegen/commit/cae980d) Default ROS2 nodes to C++, add Makefile launch targets, fix micro-ROS build

* [1defd1c](https://github.com/sireum/hamr-codegen/commit/1defd1c) update submodule

* [0aef7c2](https://github.com/sireum/hamr-codegen/commit/0aef7c2) ros2: generate Python launch files alongside XML

* [fc797e6](https://github.com/sireum/hamr-codegen/commit/fc797e6) update submodule

* [f3b0d06](https://github.com/sireum/hamr-codegen/commit/f3b0d06) ros2: fix exec_depend indentation in the generated bringup package.xml

* [b022667](https://github.com/sireum/hamr-codegen/commit/b022667) update submodule

* [11c6eb3](https://github.com/sireum/hamr-codegen/commit/11c6eb3) ros2: launch config seams, micro-ROS storage seam, checked rcl init

* [a40bd23](https://github.com/sireum/hamr-codegen/commit/a40bd23) update submodule

* [ffc544b](https://github.com/sireum/hamr-codegen/commit/ffc544b) ros2: platform-provided types, model-carried naming, micro-ROS buffers

* [9114acd](https://github.com/sireum/hamr-codegen/commit/9114acd) ros2: rcutils micro-ROS logging + Ros_Node_Kind rename

* [ea29e49](https://github.com/sireum/hamr-codegen/commit/ea29e49) add link to hamr website

* [9af43e8](https://github.com/sireum/hamr-codegen/commit/9af43e8) update changelog
...
```
export SIREUM_V=4.20260720.6a05e505
```
...
```
export SIREUM_V=4.20260115.7c92e7f9
```
...
* [263f6bc](https://github.com/sireum/hamr-codegen/commit/263f6bc) update build props

* [cf4b230](https://github.com/sireum/hamr-codegen/commit/cf4b230) microkit: add c api artifacts to codegen report

* [6e89eb6](https://github.com/sireum/hamr-codegen/commit/6e89eb6) update submodule

* [8fcd52c](https://github.com/sireum/hamr-codegen/commit/8fcd52c) microkit: Remove dependency on microkit for verus and testing targets. Added clean target for rust components.

* [de1d587](https://github.com/sireum/hamr-codegen/commit/de1d587) update submodule

* [bba1348](https://github.com/sireum/hamr-codegen/commit/bba1348) microkit: don't pass sel4 include dirs when verifying

* [0569472](https://github.com/sireum/hamr-codegen/commit/0569472) update changelog
...
```
export SIREUM_V=4.20250924.c877daf
```
...
<br>
