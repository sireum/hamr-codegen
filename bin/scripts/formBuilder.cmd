...
val homeBin: Os.Path = Os.slashDir.up
val home: Os.Path = homeBin.up
val kekinianHome = home.up.up
val sireum: Os.Path = homeBin / (if (Os.isWin) "sireum.bat" else "sireum")
val sireumJar: Os.Path = homeBin / "sireum.jar"

assert (sireum.exists, s"Expected to find $sireum")

val formsDir: Os.Path = kekinianHome.up / "forms"
if (!formsDir.exists) {
  proc"git clone https://github.com/sireum/forms.git $formsDir".console.echo.runCheck()
}

if (!sireumJar.isSymLink) {
  val sireumjar = kekinianHome / "bin" / "sireum.jar"
  if (sireumjar.exists) {
...
def process(tool: CliOpt.Tool, optionName: String): Unit = {
  ...
  def addOptions(opts: ISZ[org.sireum.cli.CliOpt.Opt], optGroup: String): Unit = {
    for (o <- opts if !ignoreKeys.contains(o.name)) {
      ...
      // tooltip the input component with the option's cli description; validation error
      // tooltips temporarily replace this (update methods restore it once valid again)
      toolTips = toolTips :+
        st"""val ${o.name}ToolTip: String = "${ops.StringOps(o.description).escapeST}"
            |${o.name}.setToolTipText(${o.name}ToolTip)"""
      o.tpe match {
        case f: Type.Flag =>
