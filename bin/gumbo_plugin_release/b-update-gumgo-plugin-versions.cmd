...
val pluginDir = Os.home / "devel" / "sireum" / "osate-plugin" / "aadl-gumbo"
val updateSiteDir = Os.home / "devel" / "sireum" / "aadl-gumbo-update-site"

val baseUpdateSite = Os.home / "devel" / "sireum" / "osate-update-site"
...
val ids: ISZ[OsateProject] = ISZ(
  OsateProject(pluginDir, "org.sireum.aadl.osate.gumbo2air", T),
  OsateProject(pluginDir, "org.sireum.aadl.gumbo", F),
)

ids.foreach((proj: OsateProject) => updateManfiest(proj))

def basePluginVersion(): String = {
  val mostRecent = ops.ISZOps(baseUpdateSite.list.filter((p: Os.Path) => p.isDir)).sortWith((a: Os.Path, b: Os.Path) => a.name >= b.name)(0)
  val o = ops.StringOps(mostRecent.name)
  val pos = o.lastIndexOf('.')
...
def updateFeatureXml(project: OsateProject): Unit ={
  ...
  val bMod = st"""      <import plugin="org.sireum.aadl.osate" version="${basePluginVersion()}" match="greaterOrEqual"/>""".render
