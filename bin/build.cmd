...
def regenClis(): Unit = {
  ...
  // fileOptionExcludes: when non-empty, a mergeOptionsM method is also generated that merges
  // file-based options into cli options; the given option names are rejected when they appear
  // in file options (e.g. line, system)
  def process(tool: CliOpt.Tool, packageName: String, outputKeyUtil: Os.Path, optionName: String,
              fileOptionExcludes: Option[ISZ[String]]): Unit = {
    ...
    def addOptions(opts: ISZ[org.sireum.cli.CliOpt.Opt], optGroup: String): Unit = {
      for (o <- opts) {
        ...
        val keyCond: ST =
          if (o.shortKey.nonEmpty) st"k == ShortKeys.$id || k == LongKeys.$id"
          else st"k == LongKeys.$id"
        if (fileOptionExcludes.nonEmpty && ops.ISZOps(fileOptionExcludes.get).contains(o.name)) {
          mergeCases = mergeCases :+
            st"""if ($keyCond) {
                |  return Either.Right("Cannot set '${o.longKey}' in file options")
                |}"""
        } else {
          var inc: Z = 2 // key + value
          o.tpe match {
            case _: Type.Flag => inc = 1 // key only
            case _ =>
          }
          mergeCases = mergeCases :+
            st"""if ($keyCond) {
                |  ret = ret(${o.name} = fileOptions.${o.name})
                |  userModifiedKeys = userModifiedKeys :+ LongKeys.$id
                |  i = i + $inc
                |}"""
        }
        o.tpe match {
    ...
    val mergeMethod: ST =
      if (fileOptionExcludes.isEmpty) st""
      else
        st"""
            |  // Merges the file-based options fileOptions (whose split representation is fileOpts) into o,
            |  // giving file options precedence. Returns the merged options along with the long keys of the
            |  // options that were set in fileOpts, or an error message if fileOpts contains non-option
            |  // arguments or options that cannot be set via file options.
            |  def mergeOptionsM(o: Cli.$optionName,
            |                    fileOptions: Cli.$optionName,
            |                    fileOpts: ISZ[String]): Either[(Cli.$optionName, ISZ[String]), String] = {
            |    assert(LongKeys.allKeys.size == ${allLongs.size}, s"Expecting ${allLongs.size} long keys but found $${LongKeys.allKeys.size}")
            |    assert(ShortKeys.allKeys.size == ${allShorts.size}, s"Expecting ${allShorts.size} short keys but found $${ShortKeys.allKeys.size}")
            |
  ...
  process(
    ...
    fileOptionExcludes = Some(ISZ("line", "system"))
  )
}
